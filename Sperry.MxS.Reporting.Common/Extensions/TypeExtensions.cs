...
namespace Sperry.MxS.Core.Common.Extensions
{
    public static class TypeExtensions
    {
        ...
        public static Dictionary<string, PropertyInfo> GetSimpleTypeProperties(this Type type, string prefix, string suffix)
        {
            Dictionary<string, PropertyInfo> simpleProperties = new Dictionary<string, PropertyInfo>();
            PropertyInfo[] typePropertyInfos = type.GetProperties();
            foreach (PropertyInfo pinfo in typePropertyInfos)
            {
                if (!pinfo.PropertyType.IsArray && pinfo.PropertyType.IsSimple())
                {
                    if (string.IsNullOrEmpty(suffix))
                    {
                        simpleProperties.Add(prefix + pinfo.Name, pinfo);
                    }
                    else
                    {
                        simpleProperties.Add(prefix + pinfo.Name + suffix, pinfo);
                    }
                }
            }
