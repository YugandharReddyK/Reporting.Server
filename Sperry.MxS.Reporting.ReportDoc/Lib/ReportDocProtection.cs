...
{
    public class ReportDocProtection
    {
        readonly int[] initialCodeArray = { 0xE1F0, 0x1D0F, 0xCC9C, 0x84C0, 0x110C, 0x0E10, 0xF1CE, 0x313E, 0x1872, 0xE139, 0xD40F, 0x84F9, 0x280C, 0xA96A, 0x4EC3 };

        readonly int[,] encryptionMatrix = new int[15, 7]
        {
        ...
        // Main implementation
        /// <summary>
        /// ApplyDocumentProtection - Encrypts password and adds security to document
        /// </summary>
        ...
        public void ApplyDocumentProtection(WordprocessingDocument wdDocument, string strPassword)
        {
            ...
            List<Table> tables = wdDocument.MainDocumentPart.Document.Descendants<Table>().ToList();
            foreach (Table tbl in tables)
            {
                var tableProperties = tbl.Descendants<TableProperties>().FirstOrDefault();
                if (tableProperties != null && tableProperties.TableCaption != null && tableProperties.TableCaption.Val.ToString() == "HALComment")
                {
                    cnt++;

                    List<TableRow> rows = tbl.Descendants<TableRow>().ToList();
                    TableRow currenTableRow = rows[0];
                    if (rows.Any())
                        currenTableRow = rows[1];

                    OpenXmlElement parent = currenTableRow.Parent;
                    ...
                    parent.InsertBefore(newPermStart, currenTableRow);

                    PermEnd newPermEnd = new PermEnd
                    {
                        Id = 100012 + cnt
                    };
                    parent.InsertAfter(newPermEnd, currenTableRow);
                }
            }
